...
router.put('/:id', async (req, res) => {
  ...
  try {
    ...
    if (!tagData[0]) {
      res.status(404).json({ message: 'No product with this id, update cannot be made!' });
      return;
    }
    res.status(200).json(tagData);
  } catch (err) {
...
router.delete('/:id', async (req, res) => {
  ...
  try {
    ...
    if (!tagData) {
      res.status(404).json({ message: 'No Product found at this id, cannot delete unknown tag!' });
      return;
    }
    res.status(200).json(tagData);
  } catch (err) {
